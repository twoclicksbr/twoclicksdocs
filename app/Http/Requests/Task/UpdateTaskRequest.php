...
class UpdateTaskRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'task_status_id'     => ['sometimes', 'required', 'integer',
                Rule::exists('tc_doc.task_statuses', 'id')
                    ->where('project_id', $projectId)->whereNull('deleted_at')],
            'task_fase_id'       => ['sometimes', 'required', 'integer',
                Rule::exists('tc_doc.task_fases', 'id')
                    ->where('project_id', $projectId)->whereNull('deleted_at')],
            'task_modulo_id'     => ['sometimes', 'required', 'integer',
                Rule::exists('tc_doc.task_modulos', 'id')
                    ->where('project_id', $projectId)->whereNull('deleted_at')],
            'task_tipo_id'       => ['sometimes', 'required', 'integer',
                Rule::exists('tc_doc.task_tipos', 'id')
                    ->where('project_id', $projectId)->whereNull('deleted_at')],
            'task_prioridade_id' => ['sometimes', 'required', 'integer',
